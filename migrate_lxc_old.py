...
def convert(data):
    """Converte e cria o container usando script shell"""
    ...
    try:
        ...
        return create_lxc_container(data)
        ...
        console.print(f"\n[red]❌ Erro inesperado durante migração: {e}[/red]")
        return False
...
def create_lxc_container(data):
    """Cria e configura o container LXC usando script shell"""
    console.print()
    console.print("[cyan]📦 Executando migração via script shell...[/cyan]")
    
    # Prepara comando para chamar o script shell
    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "migrate_container.sh")
    
    # Constrói comando com todos os parâmetros
    shell_command = [
        script_path,
        "-n", data["name"],
        "-t", data["target"],
    ...
    try:
        ...
        result = subprocess.run(shell_command, capture_output=True, text=True, timeout=1800)  # 30 min timeout
        ...
        if result.returncode == 0:
            ...
            console.print()
            console.print(f"[red]❌ Erro na migração:[/red]")
            console.print(result.stderr)
            return False
            
    except subprocess.TimeoutExpired:
        console.print()
        console.print("[red]❌ Timeout: A migração demorou muito tempo[/red]")
        return False
    except Exception as e:
        console.print()
